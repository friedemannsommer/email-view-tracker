pub fn get_default_headers_middleware() -> actix_web::middleware::DefaultHeaders {
    actix_web::middleware::DefaultHeaders::new()
        .add((
            ...
            "default-src 'none'; block-all-mixed-content; img-src data: 'self'; style-src 'self'; prefetch-src 'self'; font-src 'self'; frame-ancestors 'none'; base-uri 'none'; form-action 'self'",
        ))
